...
** 240918 CR Added Raylib support, updated to C99.
**
***************************************************************/
...
#define LOG_CHANGE(variable)                                                   \
  {                                                                            \
    printf("Changed %s at %s:%d: New Value = %ld\n", #variable, __FILE__,      \
           __LINE__, (long)variable);                                          \
  }

// Macro to print an ASCII memory diagram of x values before and after a
// pointer, displaying both hexadecimal and decimal representations
#define PrintMemoryDiagram(ptr, x)                                             \
  do {                                                                         \
...
#define M_BRANCH                                                               \
  { InsPtr = (cell_t *)(((uint8_t *)InsPtr) + READ_CELL_DIC(InsPtr)); }
...
static void TraceNames(ExecToken Token, cell_t Level) {
  ...
  if (ffTokenToName(Token, &DebugName)) {
    ...
    ffDotS();
    /* No longer needed?        gCurrentTask->td_OUT = 0; */ /* !!! Hack for
                                                                ffDotS() */
...
static int UdIsUint64(ucell_t Hi) {
  return ((2 * sizeof(ucell_t) == sizeof(uint64_t)) ? TRUE : Hi == 0);
}
...
#define PF_MEMORY_VALIDATOR (0xA81B4D69)
    case ID_ALLOCATE:
      ...
      if (CellPtr) {
        /* This was broken into two steps because different compilers
        *incremented
    ...
    case ID_ARSHIFT:
      BINARY_OP(>>);
      endcase; /* Arithmetic right shift */
    ...
    case ID_CALL_C:
      ...
      CallUserFunction(Scratch & 0xFFFF, (Scratch >> 31) & 1,
                       (Scratch >> 24) & 0x7F);
      LOAD_REGISTERS;
      endcase;
...
#define HIGH_BIT ((ucell_t)1 << (NBITS - 1))
      ...
      /* Perform cell*cell bit multiply for a 2 cell result, by factoring into
       * half cell quantities. Using an improved algorithm suggested by Steve
       * Green. Converted to 64-bit by Aleksej Saushev.
       */
    ...
    case ID_D_MTIMES: /* M* ( a b -- pl ph ) */
    {
      ...
      if ((cell_t)ahi < 0)
        ahi = (ucell_t)(-(cell_t)ahi);
      if ((cell_t)bhi < 0)
        bhi = (ucell_t)(-(cell_t)bhi);
...
#define DULT(du1l, du1h, du2l, du2h)                                           \
  ((du2h < du1h) ? FALSE : ((du2h == du1h) ? (du1l < du2l) : TRUE))
      /* Perform 2 cell by 1 cell divide for 1 cell result and remainder, using
       * shift and subtract. */
    case ID_D_UMSMOD: /* UM/MOD ( al ah bdiv -- rem q ) */
    {
      ...
      /* Perform 2 cell by 1 cell divide for 2 cell result and remainder, using
       * shift and subtract. */
...
#undef bdiv
    ...
    case ID_EXECUTE: {
      ...
      if (IsTokenPrimitive(TOS)) {
        WRITE_CELL_DIC((cell_t *)&FakeSecondary[0],
                       TOS); /* Build a fake secondary and execute it. */
    ...
    case ID_FILE_RESIZE: /* ( ud fileid -- ior ) */
    {
      ...
      TOS = (UdIsUint64(SizeHi) ? sdResizeFile(File, UdToUint64(SizeLo, SizeHi))
                                : THROW_RESIZE_FILE);
    }
      endcase;
...
#define LOCAL_STORE_N(num)                                                     \
  case ID_LOCAL_STORE_##num: /* ( n <local> -- , store n in local ) */         \
    *(LocalsPtr - (num)) = TOS;                                                \
    ...
    case ID_RESIZE: /* ( addr1 u -- addr2 result ) */
    {
      ...
      if (((ucell_t)*FreePtr) != ((ucell_t)FreePtr ^ PF_MEMORY_VALIDATOR)) {
        ...
        if (CellPtr) {
          ...
          *CellPtr =
              (cell_t)(((ucell_t)CellPtr) ^ (ucell_t)PF_MEMORY_VALIDATOR);
          /* 090218 - Fixed bug that was incrementing the address twice. Thanks
           * Reinhold Straub. */
    ...
    case ID_THROW: /* ( k*x err -- k*x | i*x err , jump to where CATCH was
                      called ) */
      if (TOS) {
        M_THROW(TOS);
      } else
    ...
    case ID_VAR_BASE:
      DO_VAR(gVarBase);
      endcase;
    case ID_VAR_BYE_CODE:
      DO_VAR(gVarByeCode);
      endcase;
    case ID_VAR_CODE_BASE:
      DO_VAR(gCurrentDictionary->dic_CodeBase);
      endcase;
    case ID_VAR_CODE_LIMIT:
      DO_VAR(gCurrentDictionary->dic_CodeLimit);
      endcase;
    case ID_VAR_CONTEXT:
      DO_VAR(gVarContext);
      endcase;
    case ID_VAR_DP:
      DO_VAR(gCurrentDictionary->dic_CodePtr.Cell);
      endcase;
    case ID_VAR_ECHO:
      DO_VAR(gVarEcho);
      endcase;
    case ID_VAR_HEADERS_BASE:
      DO_VAR(gCurrentDictionary->dic_HeaderBase);
      endcase;
    case ID_VAR_HEADERS_LIMIT:
      DO_VAR(gCurrentDictionary->dic_HeaderLimit);
      endcase;
    case ID_VAR_HEADERS_PTR:
      DO_VAR(gCurrentDictionary->dic_HeaderPtr);
      endcase;
    case ID_VAR_NUM_TIB:
      DO_VAR(gCurrentTask->td_SourceNum);
      endcase;
    case ID_VAR_OUT:
      DO_VAR(gCurrentTask->td_OUT);
      endcase;
    case ID_VAR_STATE:
      DO_VAR(gVarState);
      endcase;
    case ID_VAR_TO_IN:
      DO_VAR(gCurrentTask->td_IN);
      endcase;
    ...
    // RAYLIB: void SetShaderValueV(Shader shader, int locIndex, const void
    // *value, int uniformType, int count);
    case XT_SET_SHADER_VALUE_V: { /* ( +shader +locIndex +value +uniformType
                                     +count -- ) */
      ERR("SetShaderValueV not implemented\n");
    } break;
    // RAYLIB: void SetShaderValueMatrix(Shader shader, int locIndex, Matrix
    // mat);
    case XT_SET_SHADER_VALUE_MATRIX: { /* ( +shader +locIndex +mat -- ) */
      ERR("SetShaderValueMatrix not implemented\n");
    } break;
    // RAYLIB: void SetShaderValueTexture(Shader shader, int locIndex, Texture2D
    // texture);
    ...
    case XT_LOAD_AUTOMATION_EVENT_LIST: { /* ( +fileName -- AutomationEventList
      ...
      // RAYLIB: AutomationEventList LoadAutomationEventList(const char
      // *fileName);
    } break;
    ...
    case XT_EXPORT_AUTOMATION_EVENT_LIST: { /* ( +list +fileName -- bool ) */
      // RAYLIB: bool ExportAutomationEventList(AutomationEventList list, const
      // char *fileName);
    } break;
    ...
    case XT_UPDATE_CAMERA_PRO: { /* ( +camera +movement +rotation +zoom -- ) */
      // RAYLIB: void UpdateCameraPro(Camera *camera, Vector3 movement, Vector3
      // rotation, float zoom);
    } break;
    //
    ...
    case XT_DRAW_LINE: { /* ( +startPosX +startPosY +endPosX +endPosY +color --
      ...
      // RAYLIB: void DrawLine(int startPosX, int startPosY, int endPosX, int
      // endPosY, Color color);
    } break;
    ...
    case XT_DRAW_LINE_EX: { /* ( +startPos +endPos +thick +color -- ) */
      // RAYLIB: void DrawLineEx(Vector2 startPos, Vector2 endPos, float thick,
      // Color color);
    } break;
    case XT_DRAW_LINE_STRIP: { /* ( +points +pointCount +color -- ) */
      // RAYLIB: void DrawLineStrip(Vector2 *points, int pointCount, Color
      // color);
    } break;
    case XT_DRAW_LINE_BEZIER: { /* ( +startPos +endPos +thick +color -- ) */
      // RAYLIB: void DrawLineBezier(Vector2 startPos, Vector2 endPos, float
      // thick, Color color);
    } break;
    case XT_DRAW_CIRCLE: { /* ( +centerX +centerY +radius +color -- ) */
      // RAYLIB: void DrawCircle(int centerX, int centerY, float radius, Color
      // color);
    } break;
    case XT_DRAW_CIRCLE_SECTOR: { /* ( +center +radius +startAngle +endAngle
                                     +segments +color -- ) */
      // RAYLIB: void DrawCircleSector(Vector2 center, float radius, float
      // startAngle, float endAngle, int segments, Color color);
    } break;
    case XT_DRAW_CIRCLE_SECTOR_LINES: { /* ( +center +radius +startAngle
                                           +endAngle +segments +color -- ) */
      // RAYLIB: void DrawCircleSectorLines(Vector2 center, float radius, float
      // startAngle, float endAngle, int segments, Color color);
    } break;
    case XT_DRAW_CIRCLE_GRADIENT: { /* ( +centerX +centerY +radius +color1
                                       +color2 -- ) */
      // RAYLIB: void DrawCircleGradient(int centerX, int centerY, float radius,
    ...
    case XT_DRAW_CIRCLE_LINES: { /* ( +centerX +centerY +radius +color -- ) */
      // RAYLIB: void DrawCircleLines(int centerX, int centerY, float radius,
      // Color color);
    } break;
    case XT_DRAW_CIRCLE_LINES_V: { /* ( +center +radius +color -- ) */
      // RAYLIB: void DrawCircleLinesV(Vector2 center, float radius, Color
      // color);
    } break;
    case XT_DRAW_ELLIPSE: { /* ( +centerX +centerY +radiusH +radiusV +color -- )
                             */
      // RAYLIB: void DrawEllipse(int centerX, int centerY, float radiusH, float
      // radiusV, Color color);
    } break;
    case XT_DRAW_ELLIPSE_LINES: { /* ( +centerX +centerY +radiusH +radiusV
                                     +color -- ) */
      // RAYLIB: void DrawEllipseLines(int centerX, int centerY, float radiusH,
      // float radiusV, Color color);
    } break;
    case XT_DRAW_RING: { /* ( +center +innerRadius +outerRadius +startAngle
                            +endAngle +segments +color -- ) */
      // RAYLIB: void DrawRing(Vector2 center, float innerRadius, float
      // outerRadius, float startAngle, float endAngle, int segments, Color
      // color);
    } break;
    case XT_DRAW_RING_LINES: { /* ( +center +innerRadius +outerRadius
                                  +startAngle +endAngle +segments +color -- ) */
      // RAYLIB: void DrawRingLines(Vector2 center, float innerRadius, float
      // outerRadius, float startAngle, float endAngle, int segments, Color
      // color);
    } break;
    case XT_DRAW_RECTANGLE: { /* ( +posX +posY +width +height +color -- ) */
      // RAYLIB: void DrawRectangle(int posX, int posY, int width, int height,
      // Color color);
    } break;
    case XT_DRAW_RECTANGLE_V: { /* ( +position +size +color -- ) */
      // RAYLIB: void DrawRectangleV(Vector2 position, Vector2 size, Color
      // color);
    } break;
    ...
    case XT_DRAW_RECTANGLE_PRO: { /* ( +rec +origin +rotation +color -- ) */
      // RAYLIB: void DrawRectanglePro(Rectangle rec, Vector2 origin, float
      // rotation, Color color);
    } break;
    case XT_DRAW_RECTANGLE_GRADIENT_V: { /* ( +posX +posY +width +height +color1
                                            +color2 -- ) */
      // RAYLIB: void DrawRectangleGradientV(int posX, int posY, int width, int
      // height, Color color1, Color color2);
    } break;
    case XT_DRAW_RECTANGLE_GRADIENT_H: { /* ( +posX +posY +width +height +color1
                                            +color2 -- ) */
      // RAYLIB: void DrawRectangleGradientH(int posX, int posY, int width, int
      // height, Color color1, Color color2);
    } break;
    case XT_DRAW_RECTANGLE_GRADIENT_EX: { /* ( +rec +col1 +col2 +col3 +col4 -- )
                                           */
      // RAYLIB: void DrawRectangleGradientEx(Rectangle rec, Color col1, Color
      // col2, Color col3, Color col4);
    } break;
    case XT_DRAW_RECTANGLE_LINES: { /* ( +posX +posY +width +height +color -- )
                                     */
      // RAYLIB: void DrawRectangleLines(int posX, int posY, int width, int
      // height, Color color);
    } break;
    case XT_DRAW_RECTANGLE_LINES_EX: { /* ( +rec +lineThick +color -- ) */
      // RAYLIB: void DrawRectangleLinesEx(Rectangle rec, float lineThick, Color
      // color);
    } break;
    case XT_DRAW_RECTANGLE_ROUNDED: { /* ( +rec +roundness +segments +color -- )
                                       */
      // RAYLIB: void DrawRectangleRounded(Rectangle rec, float roundness, int
      // segments, Color color);
    } break;
    case XT_DRAW_RECTANGLE_ROUNDED_LINES: { /* ( +rec +roundness +segments
                                               +lineThick +color -- ) */
      // RAYLIB: void DrawRectangleRoundedLines(Rectangle rec, float roundness,
      // int segments, float lineThick, Color color);
    } break;
    case XT_DRAW_TRIANGLE: { /* ( +v1 +v2 +v3 +color -- ) */
      // RAYLIB: void DrawTriangle(Vector2 v1, Vector2 v2, Vector2 v3, Color
      // color);
    } break;
    case XT_DRAW_TRIANGLE_LINES: { /* ( +v1 +v2 +v3 +color -- ) */
      // RAYLIB: void DrawTriangleLines(Vector2 v1, Vector2 v2, Vector2 v3,
      // Color color);
    } break;
    case XT_DRAW_TRIANGLE_FAN: { /* ( +points +pointCount +color -- ) */
      // RAYLIB: void DrawTriangleFan(Vector2 *points, int pointCount, Color
      // color);
    } break;
    case XT_DRAW_TRIANGLE_STRIP: { /* ( +points +pointCount +color -- ) */
      // RAYLIB: void DrawTriangleStrip(Vector2 *points, int pointCount, Color
      // color);
    } break;
    case XT_DRAW_POLY: { /* ( +center +sides +radius +rotation +color -- ) */
      // RAYLIB: void DrawPoly(Vector2 center, int sides, float radius, float
      // rotation, Color color);
    } break;
    case XT_DRAW_POLY_LINES: { /* ( +center +sides +radius +rotation +color -- )
                                */
      // RAYLIB: void DrawPolyLines(Vector2 center, int sides, float radius,
      // float rotation, Color color);
    } break;
    case XT_DRAW_POLY_LINES_EX: { /* ( +center +sides +radius +rotation
                                     +lineThick +color -- ) */
      // RAYLIB: void DrawPolyLinesEx(Vector2 center, int sides, float radius,
    ...
    case XT_DRAW_SPLINE_LINEAR: { /* ( +points +pointCount +thick +color -- ) */
      // RAYLIB: void DrawSplineLinear(Vector2 *points, int pointCount, float
      // thick, Color color);
    } break;
    case XT_DRAW_SPLINE_BASIS: { /* ( +points +pointCount +thick +color -- ) */
      // RAYLIB: void DrawSplineBasis(Vector2 *points, int pointCount, float
      // thick, Color color);
    } break;
    case XT_DRAW_SPLINE_CATMULL_ROM: { /* ( +points +pointCount +thick +color --
                                          ) */
      // RAYLIB: void DrawSplineCatmullRom(Vector2 *points, int pointCount,
      // float thick, Color color);
    } break;
    case XT_DRAW_SPLINE_BEZIER_QUADRATIC: { /* ( +points +pointCount +thick
                                               +color -- ) */
      // RAYLIB: void DrawSplineBezierQuadratic(Vector2 *points, int pointCount,
      // float thick, Color color);
    } break;
    case XT_DRAW_SPLINE_BEZIER_CUBIC: { /* ( +points +pointCount +thick +color
                                           -- ) */
      // RAYLIB: void DrawSplineBezierCubic(Vector2 *points, int pointCount,
      // float thick, Color color);
    } break;
    case XT_DRAW_SPLINE_SEGMENT_LINEAR: { /* ( +p1 +p2 +thick +color -- ) */
      // RAYLIB: void DrawSplineSegmentLinear(Vector2 p1, Vector2 p2, float
      // thick, Color color);
    } break;
    case XT_DRAW_SPLINE_SEGMENT_BASIS: { /* ( +p1 +p2 +p3 +p4 +thick +color -- )
                                          */
      // RAYLIB: void DrawSplineSegmentBasis(Vector2 p1, Vector2 p2, Vector2 p3,
      // Vector2 p4, float thick, Color color);
    } break;
    case XT_DRAW_SPLINE_SEGMENT_CATMULL_ROM: { /* ( +p1 +p2 +p3 +p4 +thick
                                                  +color -- ) */
      // RAYLIB: void DrawSplineSegmentCatmullRom(Vector2 p1, Vector2 p2,
      // Vector2 p3, Vector2 p4, float thick, Color color);
    } break;
    case XT_DRAW_SPLINE_SEGMENT_BEZIER_QUADRATIC: { /* ( +p1 +c2 +p3 +thick
                                                       +color -- ) */
      // RAYLIB: void DrawSplineSegmentBezierQuadratic(Vector2 p1, Vector2 c2,
      // Vector2 p3, float thick, Color color);
    } break;
    case XT_DRAW_SPLINE_SEGMENT_BEZIER_CUBIC: { /* ( +p1 +c2 +c3 +p4 +thick
                                                   +color -- ) */
      // RAYLIB: void DrawSplineSegmentBezierCubic(Vector2 p1, Vector2 c2,
      // Vector2 c3, Vector2 p4, float thick, Color color);
    } break;
    //
    // rshapes - Spline segment point evaluation functions, for a given t [0.0f
    // .. 1.0f]
    case XT_GET_SPLINE_POINT_LINEAR: { /* ( +startPos +endPos +t -- Vector2 ) */
      // RAYLIB: Vector2 GetSplinePointLinear(Vector2 startPos, Vector2 endPos,
      // float t);
    } break;
    case XT_GET_SPLINE_POINT_BASIS: { /* ( +p1 +p2 +p3 +p4 +t -- Vector2 ) */
      // RAYLIB: Vector2 GetSplinePointBasis(Vector2 p1, Vector2 p2, Vector2 p3,
      // Vector2 p4, float t);
    } break;
    case XT_GET_SPLINE_POINT_CATMULL_ROM: { /* ( +p1 +p2 +p3 +p4 +t -- Vector2 )
                                             */
      // RAYLIB: Vector2 GetSplinePointCatmullRom(Vector2 p1, Vector2 p2,
      // Vector2 p3, Vector2 p4, float t);
    } break;
    case XT_GET_SPLINE_POINT_BEZIER_QUAD: { /* ( +p1 +c2 +p3 +t -- Vector2 ) */
      // RAYLIB: Vector2 GetSplinePointBezierQuad(Vector2 p1, Vector2 c2,
      // Vector2 p3, float t);
    } break;
    case XT_GET_SPLINE_POINT_BEZIER_CUBIC: { /* ( +p1 +c2 +c3 +p4 +t -- Vector2
                                                ) */
      // RAYLIB: Vector2 GetSplinePointBezierCubic(Vector2 p1, Vector2 c2,
      // Vector2 c3, Vector2 p4, float t);
    } break;
    //
    ...
    case XT_CHECK_COLLISION_CIRCLES: { /* ( +center1 +radius1 +center2 +radius2
                                          -- bool ) */
      // RAYLIB: bool CheckCollisionCircles(Vector2 center1, float radius1,
      // Vector2 center2, float radius2);
    } break;
    case XT_CHECK_COLLISION_CIRCLE_REC: { /* ( +center +radius +rec -- bool ) */
      // RAYLIB: bool CheckCollisionCircleRec(Vector2 center, float radius,
      // Rectangle rec);
    } break;
    ...
    case XT_CHECK_COLLISION_POINT_CIRCLE: { /* ( +point +center +radius -- bool
      ...
      // RAYLIB: bool CheckCollisionPointCircle(Vector2 point, Vector2 center,
      // float radius);
    } break;
    case XT_CHECK_COLLISION_POINT_TRIANGLE: { /* ( +point +p1 +p2 +p3 -- bool )
                                               */
      // RAYLIB: bool CheckCollisionPointTriangle(Vector2 point, Vector2 p1,
      // Vector2 p2, Vector2 p3);
    } break;
    case XT_CHECK_COLLISION_POINT_POLY: { /* ( +point +points +pointCount --
                                             bool ) */
      // RAYLIB: bool CheckCollisionPointPoly(Vector2 point, Vector2 *points,
      // int pointCount);
    } break;
    case XT_CHECK_COLLISION_LINES: { /* ( +startPos1 +endPos1 +startPos2
                                        +endPos2 +collisionPoint -- bool ) */
      // RAYLIB: bool CheckCollisionLines(Vector2 startPos1, Vector2 endPos1,
      // Vector2 startPos2, Vector2 endPos2, Vector2 *collisionPoint);
    } break;
    case XT_CHECK_COLLISION_POINT_LINE: { /* ( +point +p1 +p2 +threshold -- bool
                                             ) */
      // RAYLIB: bool CheckCollisionPointLine(Vector2 point, Vector2 p1, Vector2
      // p2, int threshold);
    } break;
    ...
    case XT_LOAD_IMAGE_RAW: { /* ( +fileName +width +height +format +headerSize
                                 -- Image ) */
      // RAYLIB: Image LoadImageRaw(const char *fileName, int width, int height,
      // int format, int headerSize);
    } break;
    case XT_LOAD_IMAGE_SVG: { /* ( +fileNameOrString +width +height -- Image )
                               */
      // RAYLIB: Image LoadImageSvg(const char *fileNameOrString, int width, int
      // height);
    } break;
    case XT_LOAD_IMAGE_ANIM: { /* ( +fileName +frames -- Image ) */
      // RAYLIB: Image LoadImageAnim(const char *fileName, int *frames);
    } break;
    case XT_LOAD_IMAGE_FROM_MEMORY: { /* ( +fileType +fileData +dataSize --
                                         Image ) */
      // RAYLIB: Image LoadImageFromMemory(const char *fileType, const unsigned
    ...
    case XT_EXPORT_IMAGE_TO_MEMORY: { /* ( +image +fileType +fileSize --
                                         unsigned char* ) */
      // RAYLIB: unsigned char *ExportImageToMemory(Image image, const char
    ...
    case XT_GEN_IMAGE_GRADIENT_LINEAR: { /* ( +width +height +direction +start
                                            +end -- Image ) */
      // RAYLIB: Image GenImageGradientLinear(int width, int height, int
      // direction, Color start, Color end);
    } break;
    case XT_GEN_IMAGE_GRADIENT_RADIAL: { /* ( +width +height +density +inner
                                            +outer -- Image ) */
      // RAYLIB: Image GenImageGradientRadial(int width, int height, float
      // density, Color inner, Color outer);
    } break;
    case XT_GEN_IMAGE_GRADIENT_SQUARE: { /* ( +width +height +density +inner
                                            +outer -- Image ) */
      // RAYLIB: Image GenImageGradientSquare(int width, int height, float
      // density, Color inner, Color outer);
    } break;
    case XT_GEN_IMAGE_CHECKED: { /* ( +width +height +checksX +checksY +col1
                                    +col2 -- Image ) */
      // RAYLIB: Image GenImageChecked(int width, int height, int checksX, int
    ...
    case XT_GEN_IMAGE_PERLIN_NOISE: { /* ( +width +height +offsetX +offsetY
                                         +scale -- Image ) */
      // RAYLIB: Image GenImagePerlinNoise(int width, int height, int offsetX,
    ...
    case XT_IMAGE_TEXT_EX: { /* ( +font +text +fontSize +spacing +tint -- Image
      ...
      // RAYLIB: Image ImageTextEx(Font font, const char *text, float fontSize,
    ...
    case XT_IMAGE_ALPHA_CLEAR: { /* ( +image +color +threshold -- ) */
      // RAYLIB: void ImageAlphaClear(Image *image, Color color, float
      // threshold);
    } break;
    ...
    case XT_IMAGE_RESIZE_CANVAS: { /* ( +image +newWidth +newHeight +offsetX
                                      +offsetY +fill -- ) */
      // RAYLIB: void ImageResizeCanvas(Image *image, int newWidth, int
      // newHeight, int offsetX, int offsetY, Color fill);
    } break;
    case XT_IMAGE_MIPMAPS: { /* ( +image -- ) */
                             // RAYLIB: void ImageMipmaps(Image *image);
    } break;
    case XT_IMAGE_DITHER: { /* ( +image +rBpp +gBpp +bBpp +aBpp -- ) */
      // RAYLIB: void ImageDither(Image *image, int rBpp, int gBpp, int bBpp,
      // int aBpp);
    } break;
    ...
    case XT_IMAGE_COLOR_REPLACE: { /* ( +image +color +replace -- ) */
      // RAYLIB: void ImageColorReplace(Image *image, Color color, Color
      // replace);
    } break;
    case XT_LOAD_IMAGE_COLORS: { /* ( +image -- Color* ) */
                                 // RAYLIB: Color *LoadImageColors(Image image);
    } break;
    case XT_LOAD_IMAGE_PALETTE: { /* ( +image +maxPaletteSize +colorCount --
                                     Color* ) */
      // RAYLIB: Color *LoadImagePalette(Image image, int maxPaletteSize, int
      // *colorCount);
    } break;
    ...
    case XT_IMAGE_DRAW_PIXEL: { /* ( +dst +posX +posY +color -- ) */
      // RAYLIB: void ImageDrawPixel(Image *dst, int posX, int posY, Color
      // color);
    } break;
    case XT_IMAGE_DRAW_PIXEL_V: { /* ( +dst +position +color -- ) */
      // RAYLIB: void ImageDrawPixelV(Image *dst, Vector2 position, Color
      // color);
    } break;
    case XT_IMAGE_DRAW_LINE: { /* ( +dst +startPosX +startPosY +endPosX +endPosY
                                  +color -- ) */
      // RAYLIB: void ImageDrawLine(Image *dst, int startPosX, int startPosY,
      // int endPosX, int endPosY, Color color);
    } break;
    case XT_IMAGE_DRAW_LINE_V: { /* ( +dst +start +end +color -- ) */
      // RAYLIB: void ImageDrawLineV(Image *dst, Vector2 start, Vector2 end,
      // Color color);
    } break;
    case XT_IMAGE_DRAW_CIRCLE: { /* ( +dst +centerX +centerY +radius +color -- )
                                  */
      // RAYLIB: void ImageDrawCircle(Image *dst, int centerX, int centerY, int
      // radius, Color color);
    } break;
    case XT_IMAGE_DRAW_CIRCLE_V: { /* ( +dst +center +radius +color -- ) */
      // RAYLIB: void ImageDrawCircleV(Image *dst, Vector2 center, int radius,
      // Color color);
    } break;
    case XT_IMAGE_DRAW_CIRCLE_LINES: { /* ( +dst +centerX +centerY +radius
                                          +color -- ) */
      // RAYLIB: void ImageDrawCircleLines(Image *dst, int centerX, int centerY,
      // int radius, Color color);
    } break;
    case XT_IMAGE_DRAW_CIRCLE_LINES_V: { /* ( +dst +center +radius +color -- )
                                          */
      // RAYLIB: void ImageDrawCircleLinesV(Image *dst, Vector2 center, int
      // radius, Color color);
    } break;
    case XT_IMAGE_DRAW_RECTANGLE: { /* ( +dst +posX +posY +width +height +color
                                       -- ) */
      // RAYLIB: void ImageDrawRectangle(Image *dst, int posX, int posY, int
      // width, int height, Color color);
    } break;
    case XT_IMAGE_DRAW_RECTANGLE_V: { /* ( +dst +position +size +color -- ) */
      // RAYLIB: void ImageDrawRectangleV(Image *dst, Vector2 position, Vector2
      // size, Color color);
    } break;
    case XT_IMAGE_DRAW_RECTANGLE_REC: { /* ( +dst +rec +color -- ) */
      // RAYLIB: void ImageDrawRectangleRec(Image *dst, Rectangle rec, Color
      // color);
    } break;
    case XT_IMAGE_DRAW_RECTANGLE_LINES: { /* ( +dst +rec +thick +color -- ) */
      // RAYLIB: void ImageDrawRectangleLines(Image *dst, Rectangle rec, int
      // thick, Color color);
    } break;
    case XT_IMAGE_DRAW: { /* ( +dst +src +srcRec +dstRec +tint -- ) */
      // RAYLIB: void ImageDraw(Image *dst, Image src, Rectangle srcRec,
      // Rectangle dstRec, Color tint);
    } break;
    case XT_IMAGE_DRAW_TEXT: { /* ( +dst +text +posX +posY +fontSize +color -- )
                                */
      // RAYLIB: void ImageDrawText(Image *dst, const char *text, int posX, int
      // posY, int fontSize, Color color);
    } break;
    case XT_IMAGE_DRAW_TEXT_EX: { /* ( +dst +font +text +position +fontSize
                                     +spacing +tint -- ) */
      // RAYLIB: void ImageDrawTextEx(Image *dst, Font font, const char *text,
    ...
    case XT_UPDATE_TEXTURE_REC: { /* ( +texture +rec +pixels -- ) */
      // RAYLIB: void UpdateTextureRec(Texture2D texture, Rectangle rec, const
      // void *pixels);
    } break;
    //
    ...
    // RAYLIB: void DrawTexture(Texture2D texture, int posX, int posY, Color
    // tint);
    ...
    case XT_DRAW_TEXTURE_V: { /* ( +texture +position +tint -- ) */
      // RAYLIB: void DrawTextureV(Texture2D texture, Vector2 position, Color
      // tint);
    } break;
    case XT_DRAW_TEXTURE_EX: { /* ( +texture +position +rotation +scale +tint --
                                  ) */
      // RAYLIB: void DrawTextureEx(Texture2D texture, Vector2 position, float
      // rotation, float scale, Color tint);
    } break;
    case XT_DRAW_TEXTURE_REC: { /* ( +texture +source +position +tint -- ) */
      // RAYLIB: void DrawTextureRec(Texture2D texture, Rectangle source,
      // Vector2 position, Color tint);
    } break;
    case XT_DRAW_TEXTURE_PRO: { /* ( +texture +source +dest +origin +rotation
                                   +tint -- ) */
      // RAYLIB: void DrawTexturePro(Texture2D texture, Rectangle source,
      // Rectangle dest, Vector2 origin, float rotation, Color tint);
    } break;
    case XT_DRAW_TEXTURE_NPATCH: { /* ( +texture +nPatchInfo +dest +origin
                                      +rotation +tint -- ) */
      // RAYLIB: void DrawTextureNPatch(Texture2D texture, NPatchInfo
      // nPatchInfo, Rectangle dest, Vector2 origin, float rotation, Color
      // tint);
    } break;
    //
    ...
    case XT_LOAD_FONT_EX: { /* ( +fileName +fontSize +codepoints +codepointCount
                               -- Font ) */
      // RAYLIB: Font LoadFontEx(const char *fileName, int fontSize, int
    ...
    case XT_LOAD_FONT_FROM_MEMORY: { /* ( +fileType +fileData +dataSize
                                        +fontSize +codepoints +codepointCount --
                                        Font ) */
      // RAYLIB: Font LoadFontFromMemory(const char *fileType, const unsigned
      // char *fileData, int dataSize, int fontSize, int *codepoints, int
      // codepointCount);
    } break;
    case XT_IS_FONT_READY: { /* ( +font -- bool ) */
                             // RAYLIB: bool IsFontReady(Font font);
    } break;
    case XT_LOAD_FONT_DATA: { /* ( +fileData +dataSize +fontSize +codepoints
                                 +codepointCount +type -- GlyphInfo* ) */
      // RAYLIB: GlyphInfo *LoadFontData(const unsigned char *fileData, int
      // dataSize, int fontSize, int *codepoints, int codepointCount, int type);
    } break;
    case XT_GEN_IMAGE_FONT_ATLAS: { /* ( +glyphs +glyphRecs +glyphCount
                                       +fontSize +padding +packMethod -- Image )
                                     */
      // RAYLIB: Image GenImageFontAtlas(const GlyphInfo *glyphs, Rectangle
      // **glyphRecs, int glyphCount, int fontSize, int padding, int
      // packMethod);
    } break;
    ...
    // RAYLIB: void DrawText(const char *text, int posX, int posY, int fontSize,
    // Color color);
    ...
    case XT_DRAW_TEXT_EX: { /* ( +font +text +position +fontSize +spacing +tint
                               -- ) */
      // RAYLIB: void DrawTextEx(Font font, const char *text, Vector2 position,
      // float fontSize, float spacing, Color tint);
    } break;
    case XT_DRAW_TEXT_PRO: { /* ( +font +text +position +origin +rotation
                                +fontSize +spacing +tint -- ) */
      // RAYLIB: void DrawTextPro(Font font, const char *text, Vector2 position,
      // Vector2 origin, float rotation, float fontSize, float spacing, Color
      // tint);
    } break;
    case XT_DRAW_TEXT_CODEPOINT: { /* ( +font +codepoint +position +fontSize
                                      +tint -- ) */
      // RAYLIB: void DrawTextCodepoint(Font font, int codepoint, Vector2
      // position, float fontSize, Color tint);
    } break;
    case XT_DRAW_TEXT_CODEPOINTS: { /* ( +font +codepoints +codepointCount
                                       +position +fontSize +spacing +tint -- )
                                     */
      // RAYLIB: void DrawTextCodepoints(Font font, const int *codepoints, int
      // codepointCount, Vector2 position, float fontSize, float spacing, Color
      // tint);
    } break;
    ...
    case XT_MEASURE_TEXT_EX: { /* ( +font +text +fontSize +spacing -- Vector2 )
                                */
      // RAYLIB: Vector2 MeasureTextEx(Font font, const char *text, float
    ...
    //
    // rtext - Text strings management functions (no UTF-8 strings, only byte
    // chars)
    ...
    case XT_TEXT_SUBTEXT: { /* ( +text +position +length -- const char* ) */
      // RAYLIB: const char *TextSubtext(const char *text, int position, int
      // length);
    } break;
    case XT_TEXT_REPLACE: { /* ( +text +replace +by -- char* ) */
      // RAYLIB: char *TextReplace(char *text, const char *replace, const char
      // *by);
    } break;
    case XT_TEXT_INSERT: { /* ( +text +insert +position -- char* ) */
      // RAYLIB: char *TextInsert(const char *text, const char *insert, int
      // position);
    } break;
    case XT_TEXT_JOIN: { /* ( +textList +count +delimiter -- const char* ) */
      // RAYLIB: const char *TextJoin(const char **textList, int count, const
      // char *delimiter);
    } break;
    case XT_TEXT_SPLIT: { /* ( +text +delimiter +count -- const char** ) */
      // RAYLIB: const char **TextSplit(const char *text, char delimiter, int
      // *count);
    } break;
    ...
    case XT_DRAW_CIRCLE_3D: { /* ( +center +radius +rotationAxis +rotationAngle
                                 +color -- ) */
      // RAYLIB: void DrawCircle3D(Vector3 center, float radius, Vector3
      // rotationAxis, float rotationAngle, Color color);
    } break;
    case XT_DRAW_TRIANGLE_3D: { /* ( +v1 +v2 +v3 +color -- ) */
      // RAYLIB: void DrawTriangle3D(Vector3 v1, Vector3 v2, Vector3 v3, Color
      // color);
    } break;
    case XT_DRAW_TRIANGLE_STRIP_3D: { /* ( +points +pointCount +color -- ) */
      // RAYLIB: void DrawTriangleStrip3D(Vector3 *points, int pointCount, Color
      // color);
    } break;
    case XT_DRAW_CUBE: { /* ( +position +width +height +length +color -- ) */
      // RAYLIB: void DrawCube(Vector3 position, float width, float height,
    ...
    case XT_DRAW_CUBE_WIRES: { /* ( +position +width +height +length +color -- )
                                */
      // RAYLIB: void DrawCubeWires(Vector3 position, float width, float height,
      // float length, Color color);
    } break;
    case XT_DRAW_CUBE_WIRES_V: { /* ( +position +size +color -- ) */
      // RAYLIB: void DrawCubeWiresV(Vector3 position, Vector3 size, Color
      // color);
    } break;
    ...
    case XT_DRAW_SPHERE_EX: { /* ( +centerPos +radius +rings +slices +color -- )
                               */
      // RAYLIB: void DrawSphereEx(Vector3 centerPos, float radius, int rings,
      // int slices, Color color);
    } break;
    case XT_DRAW_SPHERE_WIRES: { /* ( +centerPos +radius +rings +slices +color
                                    -- ) */
      // RAYLIB: void DrawSphereWires(Vector3 centerPos, float radius, int
      // rings, int slices, Color color);
    } break;
    case XT_DRAW_CYLINDER: { /* ( +position +radiusTop +radiusBottom +height
                                +slices +color -- ) */
      // RAYLIB: void DrawCylinder(Vector3 position, float radiusTop, float
      // radiusBottom, float height, int slices, Color color);
    } break;
    case XT_DRAW_CYLINDER_EX: { /* ( +startPos +endPos +startRadius +endRadius
                                   +sides +color -- ) */
      // RAYLIB: void DrawCylinderEx(Vector3 startPos, Vector3 endPos, float
      // startRadius, float endRadius, int sides, Color color);
    } break;
    case XT_DRAW_CYLINDER_WIRES: { /* ( +position +radiusTop +radiusBottom
                                      +height +slices +color -- ) */
      // RAYLIB: void DrawCylinderWires(Vector3 position, float radiusTop, float
      // radiusBottom, float height, int slices, Color color);
    } break;
    case XT_DRAW_CYLINDER_WIRES_EX: { /* ( +startPos +endPos +startRadius
                                         +endRadius +sides +color -- ) */
      // RAYLIB: void DrawCylinderWiresEx(Vector3 startPos, Vector3 endPos,
      // float startRadius, float endRadius, int sides, Color color);
    } break;
    case XT_DRAW_CAPSULE: { /* ( +startPos +endPos +radius +slices +rings +color
                               -- ) */
      // RAYLIB: void DrawCapsule(Vector3 startPos, Vector3 endPos, float
      // radius, int slices, int rings, Color color);
    } break;
    case XT_DRAW_CAPSULE_WIRES: { /* ( +startPos +endPos +radius +slices +rings
                                     +color -- ) */
      // RAYLIB: void DrawCapsuleWires(Vector3 startPos, Vector3 endPos, float
      // radius, int slices, int rings, Color color);
    } break;
    ...
    case XT_DRAW_MODEL: { /* ( +model +position +scale +tint -- ) */
      // RAYLIB: void DrawModel(Model model, Vector3 position, float scale,
      // Color tint);
    } break;
    case XT_DRAW_MODEL_EX: { /* ( +model +position +rotationAxis +rotationAngle
                                +scale +tint -- ) */
      // RAYLIB: void DrawModelEx(Model model, Vector3 position, Vector3
      // rotationAxis, float rotationAngle, Vector3 scale, Color tint);
    } break;
    case XT_DRAW_MODEL_WIRES: { /* ( +model +position +scale +tint -- ) */
      // RAYLIB: void DrawModelWires(Model model, Vector3 position, float scale,
      // Color tint);
    } break;
    case XT_DRAW_MODEL_WIRES_EX: { /* ( +model +position +rotationAxis
                                      +rotationAngle +scale +tint -- ) */
      // RAYLIB: void DrawModelWiresEx(Model model, Vector3 position, Vector3
      // rotationAxis, float rotationAngle, Vector3 scale, Color tint);
    } break;
    ...
    case XT_DRAW_BILLBOARD: { /* ( +camera +texture +position +size +tint -- )
                               */
      // RAYLIB: void DrawBillboard(Camera camera, Texture2D texture, Vector3
      // position, float size, Color tint);
    } break;
    case XT_DRAW_BILLBOARD_REC: { /* ( +camera +texture +source +position +size
                                     +tint -- ) */
      // RAYLIB: void DrawBillboardRec(Camera camera, Texture2D texture,
      // Rectangle source, Vector3 position, Vector2 size, Color tint);
    } break;
    case XT_DRAW_BILLBOARD_PRO: { /* ( +camera +texture +source +position +up
                                     +size +origin +rotation +tint -- ) */
      // RAYLIB: void DrawBillboardPro(Camera camera, Texture2D texture,
      // Rectangle source, Vector3 position, Vector3 up, Vector2 size, Vector2
    ...
    case XT_UPDATE_MESH_BUFFER: { /* ( +mesh +index +data +dataSize +offset -- )
                                   */
      // RAYLIB: void UpdateMeshBuffer(Mesh mesh, int index, const void *data,
    ...
    case XT_DRAW_MESH_INSTANCED: { /* ( +mesh +material +transforms +instances
                                      -- ) */
      // RAYLIB: void DrawMeshInstanced(Mesh mesh, Material material, const
    ...
    case XT_GEN_MESH_PLANE: { /* ( +width +length +resX +resZ -- Mesh ) */
      // RAYLIB: Mesh GenMeshPlane(float width, float length, int resX, int
      // resZ);
    } break;
    ...
    case XT_GEN_MESH_TORUS: { /* ( +radius +size +radSeg +sides -- Mesh ) */
      // RAYLIB: Mesh GenMeshTorus(float radius, float size, int radSeg, int
      // sides);
    } break;
    case XT_GEN_MESH_KNOT: { /* ( +radius +size +radSeg +sides -- Mesh ) */
      // RAYLIB: Mesh GenMeshKnot(float radius, float size, int radSeg, int
      // sides);
    } break;
    ...
    case XT_LOAD_MATERIALS: { /* ( +fileName +materialCount -- Material* ) */
      // RAYLIB: Material *LoadMaterials(const char *fileName, int
      // *materialCount);
    } break;
    ...
    case XT_SET_MATERIAL_TEXTURE: { /* ( +material +mapType +texture -- ) */
      // RAYLIB: void SetMaterialTexture(Material *material, int mapType,
      // Texture2D texture);
    } break;
    case XT_SET_MODEL_MESH_MATERIAL: { /* ( +model +meshId +materialId -- ) */
      // RAYLIB: void SetModelMeshMaterial(Model *model, int meshId, int
      // materialId);
    } break;
    // rmodles - Model animations loading/unloading functions
    case XT_LOAD_MODEL_ANIMATIONS: { /* ( +fileName +animCount --
                                        ModelAnimation* ) */
      // RAYLIB: ModelAnimation *LoadModelAnimations(const char *fileName, int
      // *animCount);
    } break;
    case XT_UPDATE_MODEL_ANIMATION: { /* ( +model +anim +frame -- ) */
      // RAYLIB: void UpdateModelAnimation(Model model, ModelAnimation anim, int
      // frame);
    } break;
    ...
    case XT_UNLOAD_MODEL_ANIMATIONS: { /* ( +animations +animCount -- ) */
      // RAYLIB: void UnloadModelAnimations(ModelAnimation *animations, int
      // animCount);
    } break;
    ...
    case XT_CHECK_COLLISION_SPHERES: { /* ( +center1 +radius1 +center2 +radius2
                                          -- bool ) */
      // RAYLIB: bool CheckCollisionSpheres(Vector3 center1, float radius1,
    ...
    case XT_CHECK_COLLISION_BOX_SPHERE: { /* ( +box +center +radius -- bool ) */
      // RAYLIB: bool CheckCollisionBoxSphere(BoundingBox box, Vector3 center,
      // float radius);
    } break;
    case XT_GET_RAY_COLLISION_SPHERE: { /* ( +ray +center +radius --
                                           RayCollision ) */
      // RAYLIB: RayCollision GetRayCollisionSphere(Ray ray, Vector3 center,
      // float radius);
    } break;
    ...
    case XT_GET_RAY_COLLISION_MESH: { /* ( +ray +mesh +transform -- RayCollision
      ...
      // RAYLIB: RayCollision GetRayCollisionMesh(Ray ray, Mesh mesh, Matrix
      // transform);
    } break;
    case XT_GET_RAY_COLLISION_TRIANGLE: { /* ( +ray +p1 +p2 +p3 -- RayCollision
                                             ) */
      // RAYLIB: RayCollision GetRayCollisionTriangle(Ray ray, Vector3 p1,
      // Vector3 p2, Vector3 p3);
    } break;
    case XT_GET_RAY_COLLISION_QUAD: { /* ( +ray +p1 +p2 +p3 +p4 -- RayCollision
                                         ) */
      // RAYLIB: RayCollision GetRayCollisionQuad(Ray ray, Vector3 p1, Vector3
    ...
    case XT_LOAD_WAVE_FROM_MEMORY: { /* ( +fileType +fileData +dataSize -- Wave
      ...
      // RAYLIB: Wave LoadWaveFromMemory(const char *fileType, const unsigned
      // char *fileData, int dataSize);
    } break;
    ...
    case XT_UPDATE_SOUND: { /* ( +sound +data +sampleCount -- void ) */
      // RAYLIB: void UpdateSound(Sound sound, const void *data, int
      // sampleCount);
    } break;
    ...
    case XT_WAVE_FORMAT: { /* ( +wave +sampleRate +sampleSize +channels -- void
      ...
      // RAYLIB: void WaveFormat(Wave *wave, int sampleRate, int sampleSize, int
      // channels);
    } break;
    ...
    case XT_LOAD_MUSIC_STREAM_FROM_MEMORY: { /* ( +fileType +data +dataSize --
                                                Music ) */
      // RAYLIB: Music LoadMusicStreamFromMemory(const char *fileType, const
    ...
    case XT_UPDATE_AUDIO_STREAM: { /* ( +stream +data +frameCount -- void ) */
      // RAYLIB: void UpdateAudioStream(AudioStream stream, const void *data,
      // int frameCount);
    } break;
